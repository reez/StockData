...
final class CacheEntry<V> {
    ...
    func isCacheExpired(after date: Date = .now) -> Bool {
        // if the date is larger than the expired it means the expired is expired
        date > expiredTimestamp
    }
    
}

extension CacheEntry: Codable where V: Codable {} // If we are going to use it for on disk
